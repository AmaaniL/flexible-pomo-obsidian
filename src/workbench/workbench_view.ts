...
export class WorkbenchItemsListView extends ItemView {
  ...
  public readonly redraw = (): void => {
    ...
    if (
      ...
      let workbenchFile: TFile = this.plugin.app.vault.getAbstractFileByPath(
        this.plugin.settings.active_workbench_path
      ) as TFile;
    ...
    if (this.data) {
      this.data.workbenchFiles.forEach((currentFile) => {
        ...
        if (
          this.plugin.opened_file_path &&
          currentFile.path === this.plugin.opened_file_path
        ) {
  ...
  private addAttributesToNavFile(
    navFile: HTMLDivElement,
  ...
  update = async (openedFile: TFile): Promise<void> => {
    ...
    for (const filePath of this.data.workbenchFiles) {
      if (
        filePath.path === openedFile.path &&
        this.plugin.timer.workItem &&
        this.plugin.timer.workItem.activeNote.path === openedFile.path
      ) {
