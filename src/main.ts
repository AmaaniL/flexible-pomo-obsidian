...
export default class FlexiblePomoTimerPlugin extends Plugin {
	...
	private registerPomodoroCommands() {
			...
			checkCallback: (checking: boolean) => {
				...
				if (this.isInactive()) {
					if (!checking) {
						this.timer = new Timer(this);
						this.timer.triggered = false;
						this.timer.extendedTime = window.moment();						this.showWorkbench();
		...
		this.addCommand({
			...
			icon: 'feather-remove',
			checkCallback: (checking: boolean) => {
				if (this.isActive() && !this.checkIfActiveTimerOn() && this.checkIfActive()) {
		...
		this.addCommand({
			...
			checkCallback: (checking: boolean) => {
				if (this.pomoWorkBench.view && this.settings.workbench_location) {
					if (!checking) {
						this.settings.workbench_location = this.settings.workbench_location === 'left' ? 'right' : 'left';
						this.app.workspace.detachLeavesOfType(WorkbenchItemsListViewType);
						this.showWorkbench();
						this.saveSettings();
					}
		...
		for (let i = 0; i <= 5; i++) {
			this.addCommand({
				...
				name: i === 0 ? 'Show Current Progress' :
					i === 1 ? 'Show All Open Tasks' :
					i === 2 ? 'Show All Tasks' :
					i === 3 ? 'Show All Open Tasks Of Active Note' :
					i === 4 ? 'Show All Tasks Of Active Note' :
					'Show Notes With Active Tasks',
				icon: 'feather-show',
				checkCallback: (checking: boolean) => {
					if (this.isActive()) {
						if (!checking) this.pomoWorkBench.current_progress_modal.openProgressModal(i);
						return true;
					}
					return false;
				}
		...
		this.addCommand({
			...
			checkCallback: (checking) => {
				if(this.settings.active_workbench_path && !checking) this.savePomoWorkBench();
				return true;
			},
		});
		...
		this.addCommand({
			...
			checkCallback: (checking) => {
				if (this.isInactive() && !checking) this.loading_suggester.insert_template();
				return true;
			},
		});
	...
	async showWorkbench() {
		const leaves = this.app.workspace.getLeavesOfType(WorkbenchItemsListViewType);
		if (leaves.length) {
			await this.app.workspace.revealLeaf(leaves[0]);
		} else {
			const leaf = this.settings.workbench_location === 'left' ?
	...
	private checkIfActive(): boolean {
		return !!this.pomoWorkBench?.data.workbenchFiles.some(f => f.path === this.getCurrentFile()?.path);
	}

	private checkIfActiveTimerOn(): boolean {
		return !!this.pomoWorkBench?.workItems.some(w => w.activeNote.path === this.getCurrentFile()?.path && w.isStartedActiveNote);
	}

	onunload() {
		try { this.timer.quitTimer(); } catch {}
		(this.app.workspace as any).unregisterHoverLinkSource(WorkbenchItemsListViewType);
	}
}
