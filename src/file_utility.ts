...
export class FileUtility {
    ...
    loadItems(filePath: string, basename: string) {
        ...
        const workbenchFile = this.plugin.app.vault.getAbstractFileByPath(normalizePath(filePath)) as TFile;
            ...
            for (const line of linePerLine) {
                if (line.startsWith("PATHS:")) {
                    ...
                    for (const csvEntry of csv) {
                        const tFile = this.plugin.app.vault.getAbstractFileByPath(normalizePath(csvEntry.trim())) as TFile;
    ...
    async handleAppend(targetFile: TFile) {
        ...
        for (const workItem of this.plugin.pomoWorkBench.workItems) {
            text += `- ${this.plugin.app.fileManager.generateMarkdownLink(workItem.activeNote, '')}\n`;
        }
        ...
        const existingContent = await this.plugin.app.vault.adapter.read(targetFile.path);
