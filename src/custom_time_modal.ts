...
export class CustomTimeModal extends Modal {
    ...
    display() {
        ...
        const pomoCustomText = new TextComponent(pomodoroEl).setValue(this._plugin.settings.pomoCustom.toString());
        textBoxEl.createEl('br');

        const shortBreakEl = textBoxEl.createSpan();
        shortBreakEl.appendText('Short Break Time (Minutes): ');
        const shortBreakText = new TextComponent(shortBreakEl).setValue(this._plugin.settings.customShortBreak.toString());
        textBoxEl.createEl('br');

        const longBreakEl = textBoxEl.createSpan();
        longBreakEl.appendText('Long Break Time (Minutes): ');
        const longBreakText = new TextComponent(longBreakEl).setValue(this._plugin.settings.customLongBreak.toString());
        textBoxEl.createEl('br');
